...
export default function CameraPermission({
  ...
  const enableCamera = async () => {
    ...
    try {
      ...
      setCameraPhase('idle');
      onCameraError(t('permission.error'));
    }
  };
  ...
  return (
    <section className={`screen camera-permission-fallback ${isOverlay ? 'guide-flow-overlay' : ''}`}>
      <main className="camera-permission-card" aria-label={t('permission.cardAria')}>
        <button className="camera-close-button" onClick={handleBack} aria-label={t('permission.closeAria')} />
        ...
        <div className="camera-permission-copy">
          <p className="camera-permission-kicker">{t('permission.kicker')}</p>
          <h1>{t('permission.title')}</h1>
          <p>{t('permission.body')}</p>
        </div>

        <button className="camera-retry-button" onClick={handleBack}>{t('permission.cta')}</button>
      </main>
    </section>
  );
}
